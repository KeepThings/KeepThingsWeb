...
export class NewMessageComponent implements OnInit {
  ...
  onSubmit() {
    this.chatService.addChat({sender_id: this.userService.user.id, receiver_id: this.data.user_id, topic: this.topicFormControl.value}).subscribe(chat => {
        this.messageService.addMessage({chat_id: chat.id, message: this.messageFormControl.value, sender_id: this.userService.user.id, send_timestamp: new Date().getTime()}).subscribe(message => {
